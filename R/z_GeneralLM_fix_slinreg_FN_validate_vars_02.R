#' @export
GeneralLM_fix_slinreg_FN_validate_vars <- function(output_list_variable_selector_rv) {
  ...
  vector_esperado <- c("x_selected", "y_selected", "vector_selected_vars",
  ...
  if (!all(vector_esperado %in% vector_observado)) {
    return(list(
      status = FALSE, 
      message = "FN control: Observed and espected names are not equal."
    ))
  }
  ##############################################################################

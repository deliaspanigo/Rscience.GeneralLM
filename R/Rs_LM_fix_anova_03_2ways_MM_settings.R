#' @export
Rs_LM_fix_anova_03_2ways_MM_settings_ui <- function(id) {
  ns <- NS(id)
  tagList(
    ...
    fluidRow(
      column(3,
             ...
             uiOutput(ns("factor01_selector")),
             uiOutput(ns("factor02_selector"))
      ),
      column(3, 
...
#' @export
Rs_LM_fix_anova_03_2ways_MM_settings_server <- function(id, my_dataset) {
  moduleServer(id, function(input, output, session) {
    ...
    output$"factor01_selector" <- renderUI({
      ns <- session$ns
    ...
    # Generar UI para el selector de factor
    output$"factor02_selector" <- renderUI({
      ns <- session$ns
    ...
    output_list <- reactive({
      ...
      result <- list(
        var_name_rv = NA,
        var_name_factor01 = NA,
        var_name_factor02 = NA,
      ...
      req(input$"var_name_rv", input$"var_name_factor01", 
          input$"var_name_factor02", input$"alpha_value")
      
      result$"var_name_rv" <- input$"var_name_rv"
      result$"var_name_factor01" <- input$"var_name_factor01"
      result$"var_name_factor02" <- input$"var_name_factor02"
      
      result$"vector_selected_vars" <- c("RV" = input$"var_name_rv",
                                         "FACTOR01" = input$"var_name_factor01",
                                         "FACTOR02" = input$"var_name_factor02")

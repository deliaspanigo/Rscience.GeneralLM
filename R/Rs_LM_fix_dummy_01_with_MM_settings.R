#' @export
Rs_LM_fix_dummy_01_with_MM_settings_ui <- function(id) {
  ns <- NS(id)
  tagList(
...
#' @export
Rs_LM_fix_dummy_01_with_MM_settings_server <- function(id, my_dataset) {
  moduleServer(id, function(input, output, session) {
    ...
    output$"dummy_selector" <- renderUI({
      ns <- session$ns
    ...
    output_list <- reactive({
      ...
      req(input$"var_name_rv", input$"var_name_dummy", 
          input$"var_name_reg", input$"alpha_value")
      
      result$"var_name_rv" <- input$"var_name_rv"
      result$"var_name_dummy" <- input$"var_name_dummy"
      result$"var_name_reg" <- input$"var_name_reg"
      
      result$"vector_selected_vars" <- c("RV" = input$"var_name_rv",
                                         "FACTOR" = input$"var_name_dummy",
                                         "REG" = input$"var_name_reg")
